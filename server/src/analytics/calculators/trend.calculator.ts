...
export class TrendCalculator {
  ...
  calculateTrends(
    ...
    const olderTopicFreq = this.calculateTopicFrequencies(olderResponses, canonicalTopics);
    const newerTopicFreq = this.calculateTopicFrequencies(newerResponses, canonicalTopics);
    ...
    const emergingTopics = this.detectEmergingTopics(olderTopicFreq, newerTopicFreq, olderResponses.length, newerResponses.length);

    // Detect declining topics (significantly less frequent in newer period)
    const decliningTopics = this.detectDecliningTopics(olderTopicFreq, newerTopicFreq, olderResponses.length, newerResponses.length);

    // Detect sentiment shifts
    const sentimentShifts = this.detectSentimentShifts(olderTopicSentiment, newerTopicSentiment);
  ...
  private calculateTopicSentiment(
    responses: ResponseDocument[]
  ): Map<string, { avgScore: number; count: number; distribution: { positive: number; neutral: number; negative: number } }> {
    const topicSentiment = new Map<string, { scores: number[]; labels: string[] }>();
    ...
    // Convert to averages
    const result = new Map<string, { avgScore: number; count: number; distribution: { positive: number; neutral: number; negative: number } }>();
  ...
  private detectEmergingTopics(
    ...
    newerFreq.forEach((newerCount, topic) => {
      ...
      // Normalize by response count to get percentages
      const olderPct = olderCount > 0 ? (olderCountForTopic / olderCount) * 100 : 0;
      const newerPct = newerCount > 0 ? (newerCount / newerCount) * 100 : 0;
      ...
      if (newerCount >= 2) {
        if (olderCountForTopic === 0) {
          ...
          const growthRate = ((newerCount - olderCountForTopic) / olderCountForTopic) * 100;
    ...
    return emerging.sort((a, b) => b.changePercentage - a.changePercentage).slice(0, 5);
  }
  ...
  private detectDecliningTopics(
    ...
    olderFreq.forEach((olderCountForTopic, topic) => {
      ...
      if (olderCountForTopic >= 2) {
        if (newerCountForTopic === 0) {
          ...
          const declineRate = ((newerCountForTopic - olderCountForTopic) / olderCountForTopic) * 100;
          if (declineRate <= -50 && olderCountForTopic >= newerCountForTopic + 2) {
            declining.push({
              topic,
              type: 'declining',
              olderMentions: olderCountForTopic,
    ...
    return declining.sort((a, b) => a.changePercentage - b.changePercentage).slice(0, 5);
  }
  ...
  private detectSentimentShifts(
    olderSentiment: Map<string, { avgScore: number; count: number; distribution: { positive: number; neutral: number; negative: number } }>,
    newerSentiment: Map<string, { avgScore: number; count: number; distribution: { positive: number; neutral: number; negative: number } }>
  ): SentimentShift[] {
    ...
    return shifts.sort((a, b) => Math.abs(b.scoreDiff) - Math.abs(a.scoreDiff)).slice(0, 5);
  }
   ...
   */
  private calculateVolumeTrend(sortedResponses: ResponseDocument[]): 'increasing' | 'stable' | 'decreasing' {
    ...
    const firstPeriodDays = this.getDaysBetween(
      new Date(firstHalf[0].submittedAt),
      new Date(firstHalf[firstHalf.length - 1].submittedAt)
    ) || 1;
    
    const secondPeriodDays = this.getDaysBetween(
