...
export class ResponseProcessor {
  ...
  // Concurrency for wave processing
  // Can be configured via environment variable
  private readonly MAX_CONCURRENCY = parseInt(process.env.ANALYTICS_MAX_CONCURRENCY || '4', 10);
  ...
  async processResponses(
    ...
    this.logger.log(`[${taskId}] Starting response processing`);
    ...
    if (emptyResponses.length > 0) {
      this.logger.log(`[${taskId}] Marking ${emptyResponses.length} empty responses as processed`);
    ...
    if (textResponses.length === 0) {
      this.logger.log(`[${taskId}] No text responses to process`);
      return {
    ...
    this.logger.log(`[${taskId}] Processing ${textResponses.length} text responses`);
    ...
    this.logger.debug(`[${taskId}] Created ${chunks.length} chunks (size: ${chunkSize}, avg length: ${avgLength})`);
    ...
    for (let i = 0; i < chunks.length; i += this.MAX_CONCURRENCY) {
      ...
      try {
        ...
        this.logger.error(`[${taskId}] Error in wave ${waveNumber}:`, error);
  ...
  private async claimUnprocessedResponses(
    ...
    this.logger.log(`[${taskId}] Found ${unprocessedResponses.length} unprocessed responses to claim`);
  ...
  private async processChunkInParallel(
    ...
    try {
      ...
      this.logger.debug(`[${taskId}] Topics result preview: ${results[0].substring(0, 200)}`);
      this.logger.debug(`[${taskId}] Raw results lengths: ${results.map(r => r.length)}`);

      const parsedTopics = JSON.parse(results[0]);
      this.logger.debug(`[${taskId}] Full topics response (first 1000 chars): ${results[0].substring(0, 1000)}`);
      ...
      this.logger.debug(`[${taskId}] Parsed structures - Topics: ${topicsArray.length}, Sentiment: ${sentimentArray.length}, Quotes: ${quotesArray.length}`);
      this.logger.debug(`[${taskId}] Extracted arrays: ${JSON.stringify({
      ...
      this.logger.error(`[${taskId}] Error in chunk ${chunkIndex}:`, error);
      throw error;
    }
  }
  ...
  private async saveChunkResults(waveResults: any[]): Promise<string[]> {
    ...
    for (const chunkResult of waveResults) {
      ...
      if (!Array.isArray(topics) || !Array.isArray(sentiment) || !Array.isArray(quotes)) {
        this.logger.error('[saveChunkResults] Invalid structure:', {
      ...
      this.logger.debug(`[saveChunkResults] Processing ${allResponseIds.size} unique responses from chunk`);
      ...
      for (const responseId of allResponseIds) {
        ...
        if (!topicData && !sentimentData && !quoteData) {
          this.logger.warn(`[saveChunkResults] No data for response ${responseId}`);
          continue;
        }
        ...
        if (topicData?.topics) {
          ...
          if (topicData.topics.length > 0) {
            this.logger.debug(`[saveChunkResults] Response ${responseId} has ${topicData.topics.length} topics: ${topicData.topics.map((t: any) => t.topic)}`);
          }
        }
    ...
    // Execute all updates
    this.logger.debug(`[saveChunkResults] Executing ${updates.length} updates...`);
    for (const { filter, update } of updates) {
      try {
        const result = await this.responseModel.updateOne(filter, update).exec();
        if (result.matchedCount === 0) {
          this.logger.warn(`[saveChunkResults] No document matched for responseId: ${filter._id}`);
        }
      } catch (error) {
        this.logger.error(`[saveChunkResults] Error updating responseId ${filter._id}:`, error);
      }
    }
    this.logger.debug(`[saveChunkResults] Completed ${updates.length} updates`);
  ...
  async releaseTaskClaim(taskId: string, formId: Types.ObjectId): Promise<void> {
    this.logger.log(`[${taskId}] Marking responses as processed for form ${formId}`);
    ...
    this.logger.log(`[${taskId}] Marked ${result.modifiedCount} responses as processed`);
  }
}
